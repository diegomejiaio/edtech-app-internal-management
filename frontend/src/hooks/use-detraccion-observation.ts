...
interface LiftDetraccionObservationParams {
  voucherId: string;
  reason?: string;
}
...
/**
 * Mutation hook to confirm a detracción observation.
 *
 * On success, updates the vouchers list cache to set detraccion_validation.lifted=true.
 ...
 */
export function useLiftDetraccionObservation() {
  ...
  return useMutation({
    mutationFn: async ({
      voucherId,
      reason,
    }: LiftDetraccionObservationParams) => {
      const token = await getToken();
      return api.patch<{
        success: boolean;
        validation_status?: VoucherValidationStatus;
      }>(
        `/vouchers/${voucherId}/lift-detraccion-observation`,
        { reason },
        { token },
      );
    },
    onSuccess: (data, { voucherId }) => {
      ...
      // Update cached vouchers list: set detraccion_validation.lifted=true and
          ...
          const pages = old.pages.map((page) => ({
            ...
            items: page.items.map((item) =>
              item.id === voucherId
                ? {
                    ...item,
                    detraccion_validation: item.detraccion_validation
                      ? { ...item.detraccion_validation, lifted: true }
                      : item.detraccion_validation,
                    ...(newValidationStatus
      ...
      queryClient.setQueryData<Voucher>(["voucher", voucherId], (old) => {
        ...
        return {
          ...old,
          detraccion_validation: old.detraccion_validation
            ? { ...old.detraccion_validation, lifted: true }
            : old.detraccion_validation,
          ...(newValidationStatus
            ? { validation_status: newValidationStatus }
            : {}),
        };
...
export type { DetraccionValidation, LiftDetraccionObservationParams };
